from os import listdir, renames, makedirs, replace
			...
			if not(card_list[x].filename in to_remove) and not(card_list[y].filename in to_remove):
				if card_list[x].stripped_filename == card_list[y].stripped_filename:
					if filecmp.cmp(paths[i] + card_list[x].filename, paths[i] + card_list[y].filename, shallow=False):
						#print("COMPARING " + paths[i] + card_list[x].filename + " WITH " + paths[i] + card_list[y].filename)
...
for card_list in card_lists:
	for card in card_list:
		#print("Moving: " + paths[i] + card.filename + " to " + cards_path + card.full_filename)
		#renames(paths[i] + card.filename, cards_path + card.full_filename)
		replace(paths[i] + card.filename, cards_path + card.full_filename)
		#print("Copying: " + paths[i] + card.filename + " to " + cards_path + card.full_filename)
